...
AUDIT_PROMPT_TEXT = """你是材质报告审核专家。根据下面解析出的报告文本和表格数据，提取关键信息并返回 JSON（不要 markdown 代码块）。

规则：
1. 所有数值必须从报告数据中读取，禁止编造。空值或"－"或"—"填空字符串。
2. 化学成分表格中标记为 Min 的行是下限，标记为 Max 的行是上限。请根据行标记填写 min 和 max 字段。
3. Rp1.0 和 Rp0.2 分开列出。每项力学测试都要列出（含 Impact Value）。
