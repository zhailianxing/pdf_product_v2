...
from app.services.pdf_utils import guess_supplier, pdf_to_base64_images, pdf_to_text_and_tables

logger = logging.getLogger(__name__)

MAX_AI_LOG_CHARS = 4000

AUDIT_PROMPT_VISION = """你是材质报告审核专家。分析报告图片，提取数据并返回 JSON（不要 markdown 代码块）。

规则：
1. 所有数值必须从报告中读取，禁止编造。报告中为空或"－"或"—"的填空字符串。
2. 化学成分表格通常有三行数据行。用 row1 记录第一行的值，row2 记录第二行的值，actual 记录第三行（实测值）。"－"或"—"填空字符串。
...
返回格式：
{
  ...
  "chemical": [{"element": "C", "actual": "", "row1": "", "row2": "", "status": "ok"}],
  "mechanical": [{"property": "Tensile Strength", "actual": "", "min": "", "max": "", "status": "ok"}]
}
"""

AUDIT_PROMPT_TEXT = """你是材质报告审核专家。根据下面的报告数据，提取关键信息并返回 JSON（不要 markdown 代码块）。

规则：
1. 化学成分和力学性能数据均已从表格中预处理好，格式为"属性: Min=值, Max=值, Actual=值"。请直接使用这些数据填写 chemical 和 mechanical 数组。"无"表示该限值不存在，填空字符串。
2. 禁止编造数值。空值或"－"或"—"或"——"填空字符串。
3. actual 与 min/max 比较：若在范围内 status 为 "ok"，否则为 "fail"。
4. supplier、certificate_number 等基本信息从报告文本中读取。
...
AUDIT_PROMPT = AUDIT_PROMPT_VISION
...
async def _audit_with_text_api(
    pdf_path: Path,
    file_name: str,
    *,
    base_url: str,
    ...
    """先用 pdfplumber 解析 PDF 为文本，再发送给文本/VL 模型。"""
    start = time.perf_counter()
    pdf_text = pdf_to_text_and_tables(pdf_path)
    ...
    endpoint = f"{base_url.rstrip('/')}/chat/completions"
    logger.info(
        "开始调用文本模型 model=%s endpoint=%s file=%s text_chars=%d timeout=%ss",
        model, endpoint, file_name, len(pdf_text), timeout,
    )
    ...
    try:
        ...
        elapsed = (time.perf_counter() - start) * 1000
        logger.exception(
            "文本模型请求异常 model=%s elapsed_ms=%.2f error=%s", model, elapsed, exc,
        )
        raise
    ...
    raw_response = response.text
    logger.info(
        "文本模型返回 status=%s model=%s elapsed_ms=%.2f body=%s",
        response.status_code, model, elapsed, _truncate_for_log(raw_response),
    )

    if response.is_error:
        logger.error(
            "文本模型调用失败 status=%s model=%s elapsed_ms=%.2f body=%s",
            response.status_code, model, elapsed, _truncate_for_log(raw_response),
        )
        response.raise_for_status()
    ...
    try:
        ...
        logger.exception(
            "文本模型返回非 JSON model=%s elapsed_ms=%.2f body=%s",
            model, elapsed, _truncate_for_log(raw_response),
        )
        raise
    ...
    text = text.strip()
    logger.info(
        "文本模型消息内容 model=%s elapsed_ms=%.2f content=%s",
        model, elapsed, _truncate_for_log(text),
    )
    if text.startswith("```"):
        text = text.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
    ...
    try:
        ...
        logger.error(
            "文本模型返回 JSON 解析失败 model=%s error=%s content=%s",
            model, exc, _truncate_for_log(text),
        )
        raise ValueError(f"AI 模型返回的内容不是合法 JSON: {exc}") from exc
...
class LocalTextAiClient(BaseAiClient):
    """本地文本模型：先用 pdfplumber 解析 PDF，再发送文本给 LLM。"""

    async def audit_pdf(self, pdf_path: Path, file_name: str) -> tuple[str, AiExtractionResult, list[str], str, float]:
        settings = get_settings()
        return await _audit_with_text_api(
            pdf_path,
            file_name,
            base_url=settings.local_base_url,
            api_key=settings.local_api_key,
