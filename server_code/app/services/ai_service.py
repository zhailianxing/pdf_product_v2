...
规则：
1. 化学成分数据已在"化学成分"部分预处理好，每个元素一行，格式为"元素: Min=值, Max=值, Actual=值"。请直接使用这些数据填写 chemical 数组，"无"表示该限值不存在，填空字符串。
2. 力学性能从报告文本中读取。Rp1.0 和 Rp0.2 分开列出。每项力学测试都要列出（含 Impact Value）。
3. 禁止编造数值。空值或"－"或"—"或"——"填空字符串。
